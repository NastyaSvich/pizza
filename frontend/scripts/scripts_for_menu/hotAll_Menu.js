function hotAll(result) {
  const outArr = document.getElementById('menuHot');
  let str = ' ';
  let count;
  if (result.length > 0) {
    for (let i = 0; i < result.length; i += 1) {
      if (result[i].category === 'Горячее') {
        count = 1;
  ...
  += `<div class='${clas}' id = ${articul}>
       <div class="center-text mb-30">
           <div class="img-200x mlr-auto pos-relative click"><img data-articul = ${articul} class = "imgForClick" src="images/pasta-1-300x300.png" alt="${name}"></div>
    ...
    if (count > 0) { outArr.innerHTML = str; } else { checkHot(); }
  } else { checkHot(); }
}
