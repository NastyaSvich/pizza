function dessertAll(result) {
  const outArr = document.getElementById('menuDessert');
  let str = ' ';
  let count;
  if (result.length > 0) {
    for (let i = 0; i < result.length; i += 1) {
      if (result[i].category === 'Десерты') {
        count = 1;
  ...
  += `<div class='${clas}' id = ${articul}>
    ...
    if (count > 0) { outArr.innerHTML = str; } else { checkDessert(); }
  } else { checkDessert(); }
}
